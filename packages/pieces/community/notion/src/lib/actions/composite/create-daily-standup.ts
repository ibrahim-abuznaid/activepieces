...
export const createDailyStandup = createAction({
  ...
  props: {
    ...
    agenda: Property.Array({
      displayName: 'Agenda Items (bullets)',
      required: false,
    }),
    ...
    const children = [
      {
        ...
        heading_2: {
          rich_text: notionAgentCommon.richText(
            propsValue.agendaHeading ?? "Yesterday's wins"
          ),
        },
      },
      ...((propsValue.agenda as string[]) ?? []).map((t) => ({
        ...
        bulleted_list_item: {
          rich_text: notionAgentCommon.richText(String(t)),
        },
      })),
      {
        object: 'block' as const,
